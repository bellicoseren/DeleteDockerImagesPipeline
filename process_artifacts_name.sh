...
do
 ...
 then
   ...
   then
     ...
     ramalabel="$(cat tag$project$repository$tag.json | jq '.labels | map(select(.color == "#1D5100" ))' | jq '.[].name' | sed 's/"//g' 2>/dev/null)"    
     fechalabel="$(cat tag$project$repository$tag.json | jq '.labels | map(select(.color == "#C92100" ))' | jq '.[].name' | sed 's/"//g' 2>/dev/null)"    
